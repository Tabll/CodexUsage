...
Codex用量 第一版真实数据源读取 Codex 桌面端本地日志，不访问网络。
...
文件监听用于近实时刷新：文件变化后先等待 5 秒防抖，再进入自动刷新队列。启动时会立即刷新一次，之后按设置中的闲置轮询间隔主动刷新；用户也可以关闭闲置主动轮询，关闭后如果文件没有变化，就不会继续自动刷新。
...
- 使用中刷新：文件变化触发的自动刷新会遵守设置中的最小间隔。
- 数据源静默不写入：闲置主动轮询开启时按设置间隔兜底刷新；关闭后不主动轮询。
