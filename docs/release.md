...
Codex用量 当前版本为 `0.2.1 (1)`，最低支持 macOS 26。
...
公开分发前还需要完成 Apple notarization。当前脚本不自动上传公证，因为需要开发者账号、App 专用密码或 API Key，以及团队的发布策略。以已经保存到钥匙串的 `<notary-profile>` profile 为例：
...
最终面向用户分发 `dist/CodexUsage-0.2.1+1.notarized.zip`。

## 安装说明

1. 解压 `dist/CodexUsage-0.2.1+1.zip`。
